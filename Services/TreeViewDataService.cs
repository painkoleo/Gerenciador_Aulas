...
namespace GerenciadorAulas.Services
{
    public class TreeViewDataService : ITreeViewDataService
    {
        ...
        public void RemoveFolder(FolderItem? selectedFolder)
        {
            ...
            // Encontrar a instância real na TreeRoot ou em seus filhos
            FolderItem? itemToRemove = FindFolderItemInTree(selectedFolder.FullPath, TreeRoot);

            if (itemToRemove == null)
            {
                LogService.LogWarning($"Não foi possível encontrar a pasta '{selectedFolder.FullPath}' na TreeRoot para remoção.");
                return;
            }

            RemoverDoHashSetRecursivo(itemToRemove);

            if (itemToRemove.ParentFolder == null)
            {
                Application.Current.Dispatcher.Invoke(() => TreeRoot.Remove(itemToRemove));
            }
            else
            {
                Application.Current.Dispatcher.Invoke(() => itemToRemove.ParentFolder.Children.Remove(itemToRemove));
                AtualizarPais(itemToRemove.ParentFolder);
            }
        ...
        private FolderItem? FindFolderItemInTree(string fullPath, ObservableCollection<object> collection)
        {
            foreach (var item in collection)
            {
                if (item is FolderItem folder)
                {
                    ...
                    var found = FindFolderItemInTree(fullPath, folder.Children);
                    if (found != null)
                    {
                        return found;
                    }
